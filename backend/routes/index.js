...
// sequelize //
const sequelize = new Sequelize("vacations_db", "root", "root", {
  host: "localhost",
  dialect: "mysql"
});
    ...
    if (!req.body.username) {
      ...
      if (user.length == 0) {
        ...
        if (user[0].role == "user") {
          ...
          let exsitUserFollow = allUsersFollow.find(
            currUser => currUser.username == user[0].username
          );
          ...
          for (let i = 0; i < allUsersFollow.length; i++) {
            if (allUsersFollow[i].username == req.session.username) {
              for (
                let t = 0;
                t < allUsersFollow[i].vacationsIdFollow.length;
                t++
              ) {
                for (let x = 0; x < vacations.length; x++) {
                  if (
                    allUsersFollow[i].vacationsIdFollow[t] == vacations[x].id
                  ) {
...
router.get("/removefollow/:vacationId", async (req, res) => {
  ...
  for (let i = 0; i < allUsersFollow.length; i++) {
    if (allUsersFollow[i].username == req.session.username) {
      let index = allUsersFollow[i].vacationsIdFollow.indexOf(
...
//_______________ edit vacation _____________________//
